"""Multi-seed partial-panel-search reruns for error bars (Stage B / E11).

Reruns the ``index_datasets.py`` -> ``partial_panel_search.py`` pipeline at a
fixed configuration (``--train-test-ratio 0.10``, matching E5's convention)
across several independent train/test holdout seeds, so the partial-panel
recall/overlap numbers can be reported as mean +/- s.d. across seeds rather
than a single point estimate -- the same pattern ``multiseed_holdout.py``
already uses for E5.
...
- Unit mode (``--dataset-path``, ``--seed``, optional ``--train-test-ratio``):
  ...
  per SLURM job (see ``slurm_jobs/run_multiseed_partial_panel_search.sbatch``).
  Both ``index_datasets.py`` and ``partial_panel_search.py`` derive their
  internal ``dataset_name`` from ``Path(dataset_path).stem`` -- so this script
  points them at a seed-suffixed *symlink* to the real dataset file, which
  gives each seed its own index pickle, covariance pickle, interval-index
  pickle, run log, and benchmark output, with zero changes to either script.

- Aggregate mode (``--aggregate``): reads every unit's
  ``results/multiseed_partial_panel_search/<dataset>/seed_<n>/query_metrics.csv``,
  computes each seed's own per-dataset summary, then the mean +/- s.d. of
  those per-seed summaries across seeds, and writes
  ``results/multiseed_partial_panel_search/summary.csv``. Cheap -- meant to
  run on the login node once every unit job has finished.
"""
...
MULTISEED_OUT_DIR = project_root / "results" / "multiseed_partial_panel_search"
SYMLINK_DIR = MULTISEED_OUT_DIR / "_dataset_symlinks"
RUN_LOG_DIR = project_root / "results" / "run_logs"
PARTIAL_PANEL_SEARCH_DIR = project_root / "results" / "partial_panel_search"


def make_seed_symlink(dataset_path: Path, seed: int) -> Path:
    """Return a symlink named ``{stem}_seed{n}.h5ad`` pointing at ``dataset_path``.

    Both ``index_datasets.py`` and ``partial_panel_search.py`` derive
    ``dataset_name`` from the path stem when given ``--dataset-paths`` --
    a seed-suffixed symlink name is the only change needed to give each seed
    its own index/covariance/interval-index/run-log/benchmark artifacts.
    """
...
def run_unit(dataset_path: Path, seed: int, train_test_ratio: float, num_queries: int) -> None:
    stem = dataset_path.stem
    seeded_path = make_seed_symlink(dataset_path, seed)
    dataset_name = seeded_path.stem  # "{stem}_seed{n}"

    with RunLogger(dataset_name=dataset_name, stage="multiseed_partial_panel_search_unit",
                    out_dir=RUN_LOG_DIR, seed=seed, train_test_ratio=train_test_ratio,
                    num_queries=num_queries):
        print(f"\n{'=' * 80}\n[multiseed_partial_panel_search] {stem} seed={seed}: building index\n{'=' * 80}")
        subprocess.run(
        ...
        print(f"\n{'=' * 80}\n[multiseed_partial_panel_search] {stem} seed={seed}: partial panel search "
              f"(num_queries={num_queries})\n{'=' * 80}")
        subprocess.run(
            [sys.executable, str(current_dir / "partial_panel_search.py"),
             "--dataset-paths", str(seeded_path),
             "--seed", str(seed),
             "--train-test-ratio", str(train_test_ratio),
        ...
        src_csv = PARTIAL_PANEL_SEARCH_DIR / dataset_name / "benchmark_interval_metrics.csv"
        ...
        print(f"[multiseed_partial_panel_search] Copied {src_csv} -> {dest_csv}")


def _per_seed_summary(df: pd.DataFrame) -> dict:
    """Reduce one seed's per-query metrics CSV to the same summary fields
    partial_panel_search.py's benchmark_summary.csv reports (mean over
    queries, and mean_speedup = mean(brute_force_time_ms) / mean(spindle_time_ms))."""
    record = {col: df[col].mean() for col in METRIC_COLUMNS}
    mean_sp = df["spindle_time_ms"].mean()
    mean_bf = df["brute_force_time_ms"].mean()
...
def main():
    parser = argparse.ArgumentParser(description="Multi-seed partial-panel-search reruns (Stage B / E11).")
    ...
    parser.add_argument("--num-queries", type=int, default=DEFAULT_NUM_QUERIES,
                         help=f"Query draws PER LENGTH BIN, passed through to partial_panel_search.py "
                              f"(default {DEFAULT_NUM_QUERIES}; see its LENGTH_BINS for the 8 bins).")
    ...
    run_unit(Path(args.dataset_path), args.seed, args.train_test_ratio, args.num_queries)
