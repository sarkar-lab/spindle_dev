...
def evaluate_brute_force_approximation(
    ...
    for i, q_dict in enumerate(test_tile_covs):
        ...
        for K in [5, 10, 20, 30, 50]:
            ...
            metric_record[f'overlap@{K}'] = round(ov, 4)
    ...
    summary_record = {
        ...
        'recall@1': round(df_query_metrics['recall@1'].mean(), 4),
        'overlap@5': round(df_query_metrics['overlap@5'].mean(), 4),
        'overlap@10': round(df_query_metrics['overlap@10'].mean(), 4),
        'overlap@20': round(df_query_metrics['overlap@20'].mean(), 4),
        'overlap@30': round(df_query_metrics['overlap@30'].mean(), 4),
        'overlap@50': round(df_query_metrics['overlap@50'].mean(), 4),
    }
    ...
    for indexed_file in indexed_files:
        ...
        if 'test_tile_covs' in saved_data:
            ...
            if not covs_file.exists():
                covs_file = project_root.parent / "Results Backup" / "split_test_indexed" / f"{dataset_name}_raw_covariances.pkl"
